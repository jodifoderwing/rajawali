<x-filament-panels::page>
    <div class="peta-kalurahan-container">
    ...
    <script>
        ...
        function setupLayers(kalkelId) {
            ...
            tanahKalurahanVector = new ol.layer.Vector({
                source: new ol.source.Vector({
                    url: kalkelId
                        ? `https://geoserver.simtaka.id/geoserver/Simtaka/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Simtaka:tanahkalurahan&outputFormat=application/json&srsname=EPSG:4326&cql_filter=kalkel_id=${kalkelId}`
                        : '',
                    format: new ol.format.GeoJSON()
                }),
                style: new ol.style.Style({
        ...
        function setupMapEventListeners() {
            map.on("singleclick", function(evt) {
                const feature = map.forEachFeatureAtPixel(evt.pixel, function(feature, layer) {
                    return layer === tanahKalurahanVector ? feature : null;
                });
        ...
        function loadKalurahanData(kalkelId) {
            const spinner = document.getElementById('loading-spinner');
            const tableSpinner = document.getElementById("loading-table-spinner");
            const noDataMessage = document.getElementById("no-data-message");
            const locationHeader = document.getElementById("location-header");
            ...
            tanahKalurahanVector.getSource().on('change', function() {
                const features = this.getFeatures();
            ...
            tanahKalurahanVector.getSource().on('error', function() {
                noDataMessage.style.display = 'block';
